...
fn main() {
    ...
    let mut house = HouseBuilder::new()
        .add_room("First room")
        .add_device(
            "Socket_1",
            SmartDevice::new_local_power_socket(&mut socket_port),
        )
        .add_device(
            "Socket_2",
            SmartDevice::new_local_power_socket(&mut socket_port),
        ...
        .add_device(
            "Thermo_1",
            SmartDevice::new_local_thermometer(&mut therm_port),
        )
        .build()
    ...
    // ============================== Room subscribers =============================
    let mut third_room = Room::default();
    third_room.subscribe(Box::new(LoggingSubscriber));
    third_room.subscribe(Box::new(|| {
        println!("lambda device added");
    }));
    house.add_room("Third room", Option::from(third_room));

    // let mut room = Room::default();
    // room.subscribe(Box::new(LoggingSubscriber::default()));
    // room.add_device("Socket_1", SmartDevice::new_local_power_socket(&mut 9999));
    // =============================================================================
